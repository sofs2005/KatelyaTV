    ...
    async function fetchConfigAndSetMenu() {
      try {
        ...
        if (config.CustomCategories && config.CustomCategories.length > 0) {
          // 检查是否已存在“分类”菜单项，避免重复添加
          const hasCustomCategory = menuItems.some(
            (item) => item.href === '/douban?type=custom'
          );
          if (!hasCustomCategory) {
            const tvboxConfigIndex = menuItems.findIndex(
              (item) => item.href === '/config'
            );
            ...
            // 将“分类”插入到“TVBox配置”之前
            if (tvboxConfigIndex !== -1) {
              newMenuItems.splice(tvboxConfigIndex, 0, customCategoryItem);
            } else {
              newMenuItems.push(customCategoryItem);
            }
            setMenuItems(newMenuItems);
          }
        }
      } catch (error) {
        console.error('Failed to fetch custom categories for sidebar:', error);
      }
    }
  ...
  return (
    <SidebarContext.Provider value={contextValue}>
      ...
      <div className='hidden md:flex'>
        <aside
          data-sidebar
          className={`fixed top-0 left-0 h-screen bg-white/40 backdrop-blur-xl transition-all duration-300 border-r border-purple-200/50 z-10 shadow-lg dark:bg-gray-900/70 dark:border-purple-700/50 ${isCollapsed ? 'w-16' : 'w-64'
            }`}
          style={{
